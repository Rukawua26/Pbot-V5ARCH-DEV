...
class TestImportsGuardrail(unittest.TestCase):
    ...
    DEPRECATED_WRAPPERS = ["learning", "notifier", "dashboard", "monitor_trades", "ml_optimizer", "test_start"]
    TARGET_DIRS = ["core", "tools", "tests"]
    ROOT_DIR = Path(__file__).resolve().parent.parent

    def test_no_legacy_imports_internally(self):
        pattern = re.compile(r"^(import\s+(" + "|".join(self.DEPRECATED_WRAPPERS) + r")|from\s+(" + "|".join(self.DEPRECATED_WRAPPERS) + r")\s+import)")
                        ...
                        with open(path, "r", encoding="utf-8") as f:
                            for i, line in enumerate(f, 1):
                                if pattern.match(line.strip()):
                                    ...
                                    if "tools." in line:
                                        continue
                                    violations.append(f"{path.relative_to(self.ROOT_DIR)}:{i} -> {line.strip()}")

        if violations:
            self.fail("Legacy root imports detected. Use tools.<module> instead:\n" + "\n".join(violations))
