...
class CorrelationReductionTests(unittest.TestCase):
    ...
    def _make_data_service(self, frames: dict[str, pd.DataFrame]):
        ds = MagicMock()
        ds.fetch_and_update_data.side_effect = (
            lambda sym, tf, fast_mode=True: frames.get(sym)
        )
    ...
    def test_high_correlation_reduces_size(self):
        ...
        with patch.object(Config, "CORRELATION_RISK_ENABLED", True), patch.object(
            Config, "CORRELATION_RISK_THRESHOLD", 0.80
        ):
            mult, details = compute_correlation_reduction(
                self.bot, "ETH/USDT", ["BTC/USDT"]
            )
    ...
    def test_insufficient_data_returns_no_reduction(self):
        ...
        with patch.object(Config, "CORRELATION_RISK_ENABLED", True), patch.object(
            Config, "CORRELATION_RISK_MIN_CANDLES", 24
        ):
            mult, details = compute_correlation_reduction(
                self.bot, "ETH/USDT", ["BTC/USDT"]
            )
