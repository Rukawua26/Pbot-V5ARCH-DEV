        ...
        with self.lock:
            ...
            for attempt in range(1, 4):  # 3 intentos
                try:
                    ...
                    last_error = e
                    print(
                        f"⚠️ [SHADOW] Intento {attempt}/3 fallido al escribir en DB: {e}"
                    )
                    if attempt < 3:
            ...
            alert_msg = (
                f"🚨 ALERTA CRÍTICA SNIPER AI 🚨\n"
                f"Fallo total de escritura en la caja negra (DB) tras 3 intentos.\n"
                f"Error: {last_error}\n"
                f"Ruta DB: {self.db_path}\n"
                f"⛔ TRADING REAL DETENIDO. Intervención manual requerida."
            )
            try:
                send_telegram_msg(alert_msg)
            except Exception as te:
                print(f"❌ [SHADOW] Fallo al enviar alerta Telegram: {te}")
...
class Brain:
    ...
    def _init_rag_cache(self):
        ...
        try:
            ...
            for r in rows:
                try:
                    ...
                    vec = [
                        snap.get("rsi", 50),
                        snap.get("adx", 20),
                        snap.get("vol_rel", 1.0) * 10,
                        snap.get("btc_delta_tf", 0.0) * 5,
                        # Nuevas dimensiones
                        snap.get("dist_ema", 0.0) * 100,
                        snap.get("z_score", 0.0),
                        snap.get("bb_pos", 0.5),
                        ob_val,
                    ]
                    # -----------------------------------------
                    matrix_data.append(vec)
                    self.rag_cache_meta.append(
                        {
                            "symbol": r["symbol"],
                            "pnl": r["pnl_percent"],
                            "date": r["timestamp"],
            ...
            if matrix_data:
                self.rag_cache_matrix = np.array(matrix_data)
                print(
                    f"🧠 RAG Vector Cache inicializado: {len(self.rag_cache_meta)} vectores en RAM."
                )
        except Exception as e:
            print(f"⚠️ Error cargando RAG Cache: {e}")

    def update_rag_cache(self, trade_data):
        """Añade un nuevo trade a la caché vectorial en tiempo real."""
        if getattr(self, "rag_cache_matrix", None) is None:
            return
        ...
        try:
            ...
            # Añadir a la matriz y metadatos
            self.rag_cache_matrix = np.vstack([self.rag_cache_matrix, vec])
            self.rag_cache_meta.append(
                {
    ...
    def load_active_trade_states(self):
        try:
            ...
            for row in rows:
                ...
                if "open_time" in state_data and isinstance(
                    state_data["open_time"], str
                ):
                    state_data["open_time"] = datetime.fromisoformat(
                        state_data["open_time"]
                    )
                loaded_states[row["symbol"]] = state_data
            return loaded_states
        except Exception as e:
            print(f"❌ Error cargando estados de trades activos: {e}")
            return {}
    ...
    def get_rag_inference(self, symbol, current_features):
        ...
        try:
            ...
            if (
                getattr(self, "rag_cache_matrix", None) is None
                or len(self.rag_cache_matrix) == 0
            ):
                return {k: 50.0 for k in ["T", "V", "C", "L", "S"]}, ["NO_CACHE"]
            ...
            curr_vec = np.array(
                [
                    current_features.get("rsi", 50),
                    current_features.get("adx", 20),
                    current_features.get("vol_rel", 1.0) * 10,  # Peso x10 al volumen
                    current_features.get("btc_delta", 0.0) * 5,  # Peso x5 a BTC
                    current_features.get("dist_ema", 0.0) * 100,
                    current_features.get("z_score", 0.0),
                    current_features.get("bb_pos", 0.5),
                    ob_val,
                ]
            )
            # -----------------------------------------
            ...
            # 2. Cálculo Vectorizado de Similitud Coseno contra toda la matriz
            dot_products = np.dot(self.rag_cache_matrix, curr_vec)
            norms_matrix = np.linalg.norm(self.rag_cache_matrix, axis=1)
            ...
            if len(valid_indices) < min_matches:
                ...
                valid_indices = top_k_idx
                valid_similarities = similarities[top_k_idx]
            else:
                # Obtener los top K índices
                valid_similarities = similarities[valid_indices]
                top_k_idx = valid_indices[
                    np.argsort(valid_similarities)[-min_matches:][::-1]
                ]

            top_matches = [self.rag_cache_meta[idx] for idx in top_k_idx]

            # Calcular Win Rate ponderado de los vecinos
            wins = sum(1 for m in top_matches if m["pnl"] > 0)
            weighted_score = (wins / len(top_matches)) * 100

            # El RAG Vectorial influye en todos los agentes por igual (Contexto Global)
            final_scores = {k: weighted_score for k in ["T", "V", "C", "L", "S"]}

            # Preparar evidencia para el log
            evidence = []
            for m in top_matches:
                evidence.append(f"{m['symbol']} ({m['pnl']:+.1f}%)")

            return final_scores, evidence
        except Exception as e:
            return {k: 50.0 for k in ["T", "V", "C", "L", "S"]}, []
