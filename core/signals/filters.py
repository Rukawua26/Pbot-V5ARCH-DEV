...
def _apply_entry_filters_and_adjust_prob(bot, symbol, symbol_raw, df_main, audit_signal, prob_final, ctx, vol_rel):
    ...
    if filter_passed and audit_signal in ["BUY", "SELL"]:
        shock_dist_pct, shock_level = bot._get_shock_distance_pct(
    ...
    if bool(getattr(Config, "BREAKOUT_WATCH_ENABLED", True)):
        breakout_ready, breakout_info = (
            bot.breakout_agent.evaluate_breakout(symbol, df_main)
        )
    ...
    if prob_ia > 0:
        prob_ia, adjustment_reason = (
            bot._calculate_quant_consensus(prob_ia, ctx)
        )
...
def _plan_execution_mode(bot, symbol, audit_signal, prob_final, audit_verdict, filter_passed, filter_reason, ctx):
    is_shadow_exec = True
    ...
    if (
        ...
        and prob_final >= max(SHADOW_MIN_THRESHOLD, float(getattr(Config, "BREAKOUT_MIN_IA_PROB", 60.0)))
    ):
    ...
    if bool(getattr(Config, "DIRECTIONAL_COHERENCE_FILTER", True)):
        ...
        extreme_breakout_ok = breakout_shadow_override and prob_final >= float(getattr(Config, "BREAKOUT_EXTREME_IA_PROB", 75.0))
        ...
        if audit_signal == "SELL" and is_bull and not extreme_breakout_ok:
            ...
            if bool(getattr(Config, "BREAKOUT_WATCH_ENABLED", True)) and bool(getattr(Config, "BREAKOUT_WATCH_COHERENCE_ENABLED", True)):
                ...
                if shock_level_coh is not None:
                    ...
                    if added_watch:
                        bot.log(f"👁️ [ACECHO:COHERENCIA] {symbol} side={audit_signal} IA={prob_final:.1f}% sentiment={sentiment_label}")
...
def _resolve_audit_verdict_and_stats(
    ...
    if symbol in bot.cooldown_pairs and datetime.now() < bot.cooldown_pairs[symbol]:
        remaining = int((bot.cooldown_pairs[symbol] - datetime.now()).total_seconds() / 60) + 1
