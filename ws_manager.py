...
class BinanceWebSocket:
    def __init__(self, symbols=["btcusdt"]):
        ...
        self.l2_state = {s: {"bid": 0.0, "ask": 0.0, "spread": 0.0} for s in self.symbols}
    ...
    def update_symbols(self, symbols):
        ...
        if set(self.symbols) != set(new_symbols):
            ...
            for s in self.symbols:
                new_state[s] = self.l2_state.get(s, {"bid": 0.0, "ask": 0.0, "spread": 0.0})
...
if __name__ == '__main__':
    ...
    for i in range(5):
        time.sleep(1)
        print(f"Tick {i+1}: BTC: {ws.get_l2_spread('BTC/USDT')}% | ETH: {ws.get_l2_spread('ETH/USDT')}%")
    ws.stop()
